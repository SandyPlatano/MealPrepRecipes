...
export interface UseSidebarBuiltinItemsOptions {
  sections: Record<string, SectionConfig>;
  setSections: React.Dispatch<React.SetStateAction<Record<string, SectionConfig>>>;
}

export interface UseSidebarBuiltinItemsReturn {
  updateBuiltInItem: (
...
/**
 * Hook for built-in sidebar section item customization.
 * Handles item updates, reordering, custom items within built-in sections.
 */
export function useSidebarBuiltinItems({
  sections,
  setSections,
}: UseSidebarBuiltinItemsOptions): UseSidebarBuiltinItemsReturn {
  ...
  const addCustomItemToBuiltInSection = React.useCallback(
    async (sectionId: BuiltInSectionId, item: Omit<CustomSectionItem, "id" | "sortOrder">) => {
      ...
      setSections((prev) => {
        ...
        const maxSortOrder = Math.max(...section.customItems.map((i) => i.sortOrder), 0);
